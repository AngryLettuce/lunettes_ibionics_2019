...
%     [X3, Y3, Z] = findReflectedVector(Vlm, NULC, Zmw);
    ...
    [X1,Y1] = angle2XY(maxAngles(1), maxAngles(3), Z, d, memsAngle);
    %left bottom (min aX, max aY)
    [X2,Y2] = angle2XY(maxAngles(1), maxAngles(4), Z, d, memsAngle);
    %right bottom (max aX, max aY)
    [X3,Y3] = angle2XY(maxAngles(2), maxAngles(4), Z, d, memsAngle);
    %top left (max aX, min aY)
    [X4,Y4] = angle2XY(maxAngles(2), maxAngles(3), Z, d, memsAngle);
